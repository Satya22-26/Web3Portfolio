...
const Projects = ({state}) => {
    ...
    {projects !== "" && projects.map((project) => {
        if (project.name !== "" && project.descrip !== "" && project.githubLink !== "" && project.imgLink !== "") {
            const githubLink = `https://github.com/Satya22-26/${project.githubLink}`;
            return (
                <a href={githubLink} className="project-card" target='_blank' rel="noopener noreferrer">
                    <div className="card-img">
                        <img src={`https://gateway.pinata.cloud/ipfs/${project.imgLink}`} alt="" />
                    </div>
 ...
 <Modal size='md' isOpen={modal} toggle={() => setModal(!modal)}>
                    ...
                    {/*  =========popup bootstrap end==========  */}
                    <p className='donate' onClick={() => setModal(true)}>Enjoyed my project's ? Consider donating Eth's <FaDonate className='icon' /></p>
        </section>
    )
}
